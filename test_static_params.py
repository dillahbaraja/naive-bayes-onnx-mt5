...
def optimize_pair(csv_file, pair_name):
    ...
    print(f"\n================ ANALISIS MANUAL STATIC MODEL FOR: {pair_name} ================")
    
    # Let's search with relaxed conditions (minimum trades = 2) to see what is possible!
    conf_options = [0.34, 0.35, 0.36, 0.37, 0.38]
                ...
                for sl_mult in sl_options:
                    for tp_mult in tp_options:
                        for use_be in be_options:
                            ...
                            if num_trades >= 3:
                                results.append({
                                    'profit': profit, 'trades': num_trades, 'wr': wr,
                                    'params': {'min_conf': min_conf, 'min_spread': min_spread, 'tz_offset': tz_offset, 'sl_mult': sl_mult, 'tp_mult': tp_mult, 'use_be': use_be}
                                })
    ...
    if len(results_df) > 0:
        ...
        if len(profitable) > 0:
            best = profitable.sort_values(by='profit', ascending=False).iloc[0]
            print(f"Profit Terbanyak (>0): Profit=${best['profit']:.2f}, Trades={best['trades']}, WR={best['wr']*100:.2f}%")
            print(f"Params: {best['params']}")
        else:
            best_loss = results_df.sort_values(by='profit', ascending=False).iloc[0]
            print(f"Semua Loss. Hasil 'paling mendingan': Profit=${best_loss['profit']:.2f}, Trades={best_loss['trades']}, WR={best_loss['wr']*100:.2f}%")
            print(f"Params: {best_loss['params']}")
    else:
        print("Sama sekali tidak ada trade di atas 3.")
