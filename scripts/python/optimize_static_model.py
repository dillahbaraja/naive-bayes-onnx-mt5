...
def simulate_ea(df, pipeline, min_conf, min_spread, tz_offset, sl_mult, tp_mult, use_be, use_trend_filter):
    ...
    # Predict probabilities using sklearn pipeline (identical to ONNX output)
    probs = pipeline.predict_proba(X) # Shape: [N, 3] -> [Hold, Buy, Sell]
    labels = pipeline.predict(X)      # Shape: [N] -> 0, 1, 2
    
    df_sim = df.copy()
    df_sim['Prob_Hold'] = probs[:, 0]
    df_sim['Prob_Buy'] = probs[:, 1]
    df_sim['Prob_Sell'] = probs[:, 2]
    df_sim['Pred_Label'] = labels
    
    # Calculate adjusted hour for features
    df_sim['Adjusted_Hour'] = (df_sim['Hour'] + tz_offset) % 24
    
    # Re-evaluate predictions with adjusted hour
    X_shifted = X.copy()
    ...
    trades = []
    active_position = None # None or {'type': 'BUY'/'SELL', 'entry_price': float, 'sl': float, 'tp': float, 'entry_bar': int, 'highest_price': float, 'lowest_price': float}
    ...
    for t in range(1, len(df_sim) - 5):
        ...
        if active_position is not None:
            ...
            if hit_sl or hit_tp or bars_held >= holding_bars:
                if not hit_sl and not hit_tp: # Time exit
                    if active_position['type'] == 'BUY':
        ...
        # 2. Check for new entries (Only if no active position)
        if active_position is None:
            ...
            # Apply confidence and spread filters
...
def optimize_pair(csv_file, pair_name):
    ...
    print(f"\n================ OPTIMASI STATIC MODEL FOR: {pair_name} ================")
    
    # Search space
    # Loosened min_conf to handle natural priors (usually between 0.34 and 0.42)
    conf_options = [0.34, 0.35, 0.36, 0.37, 0.38, 0.39, 0.40]
    spread_options = [0.0, 0.01, 0.02, 0.03, 0.05, 0.08]
    tz_options = [0, 1]
                ...
                for sl_mult in sl_options:
                    for tp_mult in tp_options:
                        for use_be in be_options:
                            ...
                            if num_trades >= 12: # At least 12 trades a year
...
def main():
    optimize_pair("EURUSD_H1_Data.csv", "EURUSD")
